...
默认情况下，各个mod已经生成，可以在 minirsvr/websvr/目录下，直接执行 go build -o ../../bin/ httpserver.go 命令，即可完成编译。
编译结束后，就会在 minirsvr/bin/ 目录里生成对应平台的可执行程序。不需要执行如下步骤。
如果想自己重新构建编译过程，则再继续下述步骤：
